...
public class CloneGraph_133 {
    ...
    public void dfs(Node node, Node copy, Node[] visited) {
        ...
        for (Node n : node.neighbors) {
            ...
            if (visited[n.val] == null) {
                ...
                //                 as the adjacent node of prev copied node
    ...
    public Node cloneGraph(Node node) {
        if (node == null) {
            return null; // if the actual node is empty there is nothing to copy, so return null
        }
        ...
        // so in the Node[] initially null is stored, if that node is visited, we will store the respective node at the index,
        // and can retrieve that easily.
